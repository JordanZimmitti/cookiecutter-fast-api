...
async def test_validation_error_handler():
    ...
    # Checks whether a valid JSONResponse instance is created correctly
    json_response = await {{cookiecutter.class_name}}Base.validation_error_handler(None, validation_error_mock)
    assert json_response.body == b'{"message":"Validation Error: Test validation-error message"}'
